...
// SearchAPI return db record for searched term
func (m *postgresDBRepo) SearchAPI(str string) string {

	sqlStatment := `SELECT * FROM recipesnonjson
		WHERE meal=$1;`

	rows, err := m.DB.Query(sqlStatment, str)
	if err != nil {
		fmt.Println(err)
	}
